function label_joints_3d(boxPath, skeletonPath)
...
easywand_path = uibrowse('*.mat',[],'Select calibration file');
easyWandData=load(easywand_path);
frame_size=176;
% frame_size=352;
allCams=HullReconstruction.Classes.all_cameras_class(easyWandData.easyWandData);
fundMats(:,:,1:2)=allCams.Fundamental_matrices(:,:,1:2);
fundMats(:,:,3)=allCams.Fundamental_matrices(:,:,1)';
fundMats(:,:,4)=allCams.Fundamental_matrices(:,:,3);
fundMats(:,:,5)=allCams.Fundamental_matrices(:,:,2)';
fundMats(:,:,6)=allCams.Fundamental_matrices(:,:,3)';
coupless=sortrows([nchoosek(1:3,2);fliplr(nchoosek(1:3,2))]);
...
zoomBoxWindow = config.zoomBoxFrames(1):config.zoomBoxFrames(2);
    ...
    function initializeLabels()
        ...
        if ~exists(labels.savePath) || recreate_labels
            ...
            labels.initialization = NaN(numel(labels.skeleton.nodes), 2,3, numFrames, 'single');
            ...
            labels.positions = NaN(numel(labels.skeleton.nodes), 2, 3,numFrames, 'single');
...
%         ui.epiLines=zeros(6,1);
        ...
        for camInd=1:3
            ...
            ui.img(camInd).fig = figure('NumberTitle','off','MenuBar','none','ToolBar','none', ...
                ...
                @keyPress, 'DeleteFcn', @quit,'color',eyeMat(camInd,:),...
                'Position', config.imgFigPos);
            ui.img(camInd).ax = axes(ui.img(camInd).fig);
            ui.img(camInd).img = imagesc(ui.img(camInd).ax, box(:,:,(3*camInd-2):(3*camInd),1));
...
%         delete(boxSinglesPath)
        ...
        preds= h5readgroup('D:\merged datasets (mine+tsvi''s)\merged_with_masks_sep_wings_264.h5');
...
initializeSkeleton();
    ...
    % Utility function that returns node positions from the corresponding graphics objects
        pos= NaN(numel(ui.skel(1).nodes),2,3);
        for camInd=1:3
            for i = 1:numel(ui.skel(camInd).nodes)
    ...
    % Updates pre-initialiazed skeleton graphics objects
        ...
        for camInd=1:3
            ...
            cz2=double(cropZone(:,coupless(2*camInd-1,1),ui.status.currentFrame));
            line=fundMats(:,:,2*camInd-1)*...
                [(cz2(2)+pos(ui.status.selectedNode,1,camInd));...
                (801-(cz2(1)+pos(ui.status.selectedNode,2,camInd)));...
                1];
            cz2=double(cropZone(:,coupless(2*camInd-1,2),ui.status.currentFrame));
            line=-line./line(2,:);                        
            try
                delete(ui.epiLines(2*camInd-1))
            end
            eyeMat=eye(3);
            ui.epiLines(2*camInd-1)=fplot(ui.img(coupless(2*camInd-1,2)).ax,...
               @(x) 801-cz2(1)-(line(1)*(x+cz2(2))+line(3)),'Color',eyeMat(camInd,:));
            set(ui.epiLines(2*camInd-1), 'PickableParts', 'none');
            ui.img(coupless(2*camInd-1,2)).ax.XLim=[0,frame_size];
            ui.img(coupless(2*camInd-1,2)).ax.YLim=[0,frame_size];

            cz2=double(cropZone(:,coupless(2*camInd,1),ui.status.currentFrame));
            line=fundMats(:,:,2*camInd)*...
                [(cz2(2)+pos(ui.status.selectedNode,1,camInd));...
                (801-(cz2(1)+pos(ui.status.selectedNode,2,camInd)));...
                1];
            cz2=double(cropZone(:,coupless(2*camInd,2),ui.status.currentFrame));
            line=-line./line(2,:);                        
            try
                delete(ui.epiLines(2*camInd))
            end
            ui.epiLines(2*camInd)=fplot(ui.img(coupless(2*camInd,2)).ax,...
               @(x) 801-cz2(1)-(line(1)*(x+cz2(2))+line(3)),'Color',eyeMat(camInd,:));
            set(ui.epiLines(2*camInd), 'PickableParts', 'none');
            ui.img(coupless(2*camInd,2)).ax.XLim=[0,frame_size];
            ui.img(coupless(2*camInd,2)).ax.YLim=[0,frame_size];
        end
        ...
        for i = 1:numNodes
            ...
            for camInd=1:3
                ...
                % Uncommitted changes
                if ui.status.movedNodes(i); ui.skel(camInd).nodes(i).Marker = 's'; end
...
%         pos(node_idx,:) = default_pos(node_idx,:);
    ...
    % Utility to compute the initial node positions for a single frame
        ...
        pos = repmat(labels.skeleton.joints.pos,[1,1,3]);
...
%% Frame update and saving
    ...
    % Utility function for seeking to another frame
        ...
        % Update image
        for camInd=1:3
            ui.img(camInd).img.CData = box(:,:,(3*camInd-2):(3*camInd),idx);
