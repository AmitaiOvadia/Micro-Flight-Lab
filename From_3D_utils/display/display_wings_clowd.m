...
%     right_inds = right_inds(1:end-1);
    ...
    path = "C:\Users\amita\OneDrive\Desktop\micro-flight-lab\micro-flight-lab\Utilities\Work_W_Leap\From_3D_utils\display\output_3D.mp4";
...
%         p(6) = plot3(points_to_display(wings_joints_inds,frame_ind,x),points_to_display(wings_joints_inds,frame_ind,y),points_to_display(wings_joints_inds,frame_ind,z),'o-b');
        ...
        k = convhulln(pnts);
        T(1) = trisurf(k,pnts(:,1),pnts(:,2),pnts(:,3),'FaceColor','cyan');

        pnts = squeeze(preds_3D_clowd(frame_ind, right, :));
        k = convhulln(pnts);
        T(2) = trisurf(k,pnts(:,1),pnts(:,2),pnts(:,3),'FaceColor','cyan');
