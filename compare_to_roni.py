...
class DataComparison:
    def __init__(self, mov_num, path_my_data, path_roni_data, smoothed):
    ...
    def load_my_data(self):
        with h5py.File(self.path_my_data, 'r') as hdf:
            ...
            self.my_data = {
                ...
                'yaw_angle': group['yaw_angle'][:].squeeze(),
                ...
                'wings_psi_right': group['wings_psi_right'][:].squeeze() + 180,
    ...
    def plot_vectors(self, my_data, roni_data, vector_name):
        ...
        for i, label in enumerate(labels):
            plt.plot(self.roni_data['frames']/convert_to_ms, roni_data[:, i], label=f'Roni {label} {vector_name}')
            plt.plot(np.arange(len(my_data))/convert_to_ms, my_data[:, i], linestyle='--', label=f'My {label} {vector_name}')
        plt.xlabel('Ms')
        plt.ylabel(f'{vector_name} Coordinate')
        plt.title(f'{vector_name} Vectors')
        plt.legend()
        plt.show()
    ...
    def compare_body_angles(self):
        ...
        for angle in angles:
            plt.figure()
            plt.plot(self.roni_data['frames'] / convert_to_ms, self.roni_data[f"{angle}_body"], label=f'Roni {angle.capitalize()}')
            plt.plot(self.my_data['frames'] / convert_to_ms, self.my_data[f'{angle}_angle'], linestyle='--', label=f'My {angle.capitalize()}')
            plt.xlabel('ms')
            plt.ylabel(f'{angle.capitalize()} Angle (degrees)')
            plt.title(f'{angle.capitalize()} Body Angle')
            plt.legend()
            plt.show()
    ...
    def compare_wings_angles(self, wing='left'):
        for angle in ['phi', 'theta', 'psi']:
            plt.figure()
            plt.plot(self.roni_data['frames'], self.roni_data[f'{angle}_{wing}'], label=f'Roni {angle.capitalize()}')
            plt.plot(self.my_data['frames'], self.my_data[f'wings_{angle}_{wing}'], linestyle='--', label=f'My {angle.capitalize()}')
            plt.xlabel('Frame')
            plt.ylabel(f'{angle.capitalize()} Angle (degrees)')
            plt.title(f'{angle.capitalize()} {wing.capitalize()} Wing Angle')
            plt.legend()
            plt.show()
...
if __name__ == "__main__":
    ...
    comparison = DataComparison(mov_num, path_my_data, path_roni_data, smoothed)
