...
class Flight3DProcessing:
    ...
    def find_3D_points_from_ensemble(base_path, test=False):
        ...
        final_score, best_points_3D, all_models_combinations = Predictor2D.find_3D_points_optimize_neighbors(all_points_list)
    ...
    def predict_all_movies(base_path, config_path_2D,
                           calibration_path, movies=None, config_functions_inds=None, already_predicted_2D=False):
        ...
        config_functions = [config_1_5, config_2_5,
                            config_3_5, config_4_5,
                            config_5_5, config_6_5,
                            config_7_5, config_8_5]
        #
        ...
        for movie_path in file_list:
            ...
            box_path = movie_path
            save_path = os.path.join(dir_path, 'movie 2D and 3D.mp4')
            movie = os.path.basename(dir_path)
            movie_hdf5_path = create_movie_analysis_h5(movie, dir_path, points_3D_path, smooth=True)
            Visualizer.create_movie_mp4(movie_hdf5_path, save_frames=None,
                                        reprojected_points_path=reprojected_points_path,
                                        box_path=box_path, save_path=save_path)
...
if __name__ == '__main__':
    ...
    cluster = True
    if cluster is True:
        base_path = 'test movies'
        calibration_path = r"/cs/labs/tsevi/amitaiovadia/pose_estimation_venv/predict/test movies/calibration file.h5"
