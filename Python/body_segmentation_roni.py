...
from scipy.ndimage import center_of_mass, shift, binary_closing, binary_opening
import matplotlib
...
class BodySegmentation:
    def __init__(self, box_path, config_path=""):
        ...
        new_file_name = os.path.join(directory, "body_segmentations.h5")
    ...
    def get_CMs_body_segmentations(self):
        ...
        for frame in tqdm(range(2*SPAN, self.num_frames - 2*SPAN)):
            for cam in range(self.num_cams):
                ...
                body_segmentation = np.where(original_image > num_channels / 1.5, original_image, 0)
                ...
                body_segmentation_cropped = binary_opening(body_segmentation_cropped.astype(bool), iterations=2)
    ...
    def get_x_body(self):
        ...
        for frame in tqdm(range(2*SPAN, self.num_frames - 2*SPAN)):
            ...
            fly_points = self.segment_body_3D_points(cube, frame)
...
if __name__ == '__main__':
    ...
    # pts_3D = np.load(r"C:\Users\amita\PycharmProjects\pythonProject\vision\train_nn_project\2D to 3D\code on cluster\selected_movies\mov10_u\points_3D_ensemble.npy")
