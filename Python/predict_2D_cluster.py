...
class Predictor2D:
    def __init__(self, configuration_path, first_frame=0, last_frame=-1):
        ...
        self.box = self.get_box()[self.first_frame:self.last_frame]
    ...
    def get_run_name(self):
        ...
        run_name = f"{name}_start_frame_{self.first_frame}_last_frame_{self.last_frame}_{self.model_type}_{date.today().strftime('%b %d')}"
        return run_name
...
def config_2(config):
    # 3 good cameras 1
    config[
        "wings pose estimation model path"] = r"models/3 good cameras/MODEL_18_POINTS_3_GOOD_CAMERAS_Jan 03/best_model.h5"
    config["model type"] = "WINGS_AND_BODY_SAME_MODEL"
    config["predict again using reprojected masks"] = 0
    return config
...
def config_3(config):
    # 3 good cameras 2
    config[
        "wings pose estimation model path"] = r"models/3 good cameras/MODEL_18_POINTS_3_GOOD_CAMERAS_Jan 03_01/best_model.h5"
    config["model type"] = "WINGS_AND_BODY_SAME_MODEL"
    config["predict again using reprojected masks"] = 0
    return config
...
def config_4(config):
    ...
    config["wings pose estimation model path"] = r"models/per wing/MODEL_18_POINTS_PER_WING_Jan 11_03/best_model.h5"
    config[
        "wings pose estimation model path second path"] = "models/per wing/MODEL_18_POINTS_PER_WING_Jan 20/best_model.h5"
    config["model type"] = "WINGS_AND_BODY_SAME_MODEL"
...
def config_5(config):
    ...
    config["wings pose estimation model path"] = r"models/per wing/MODEL_18_POINTS_PER_WING_Jan 11_03/best_model.h5"
    config[
        "wings pose estimation model path second path"] = r"models/4 cameras/concatenated encoder/ALL_CAMS_18_POINTS_Jan 19_01/best_model.h5"
    config["model type"] = "WINGS_AND_BODY_SAME_MODEL"
...
def config_6(config):
    ...
    config["wings pose estimation model path"] = r"models/per wing/MODEL_18_POINTS_PER_WING_Jan 11_03/best_model.h5"
    config[
        "wings pose estimation model path second path"] = r"models/4 cameras/concatenated encoder/ALL_CAMS_18_POINTS_Jan 20_01/best_model.h5"
    config["model type"] = "WINGS_AND_BODY_SAME_MODEL"
    config["model type second pass"] = "ALL_CAMS_PER_WING"
    config["predict again using reprojected masks"] = 1
    return config
...
def predict_all_movies(base_path, config_path_2D):
    ...
    # movies = ["mov5", "mov24", "mov30", "mov36",
    #           "mov41", "mov43", "mov45", "mov46", "mov54", "mov57", "mov58", "mov72", "mov75"]
    # moives = os.listdir(base_path)
    ...
    for movie_path in file_list:
        ...
        for model in range(len(config_functions)):
            ...
            try:
                list_of_frames = movies_list[directory_name]
                for first_frame, last_frame in list_of_frames:
                    predictor = Predictor2D(new_config_path, first_frame=first_frame, last_frame=last_frame)
...
movies_list = {
               ...
               'mov32': [(0, 750), (750, -1)],
               'mov35': [(0, 750), (750, -1)],
               'mov42': [(0, 1000), (1000, 2000), (2000, -1)],
               'mov53': [(0, 800), (800, 1600), (1600, -1)],
               'mov61': [(0, 1000), (1000, 2000), (2000, 3000), (3000, 4000), (4000, -1)],
               'mov62': [(0, 800), (800, -1)]
               }
