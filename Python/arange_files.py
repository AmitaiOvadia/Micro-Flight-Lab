...
source_dir = fr"G:\My Drive\Amitai\Bees + UV + magnetic field 14-9\all movies"

movies_dir = r"G:\My Drive\Amitai\Bees + UV + magnetic field 14-9"
...
def arange_files():
    ...
    for mov in sparse_cam1:
        ...
        new_dir = os.path.join(movies_dir, 'movies + README', f'mov{mov_count}')
        readme_path = f'{new_dir}\README_mov{mov_count}.txt'
        copy_movies(hour, minute, mov_count, movies_paths, new_dir, readme_path, substring, uv)
        mov_count += 1


def copy_movies(hour, minute, mov_count, movies_paths, new_dir, readme_path, substring, uv):
    global i, src_path
    ...
    for i, src_path in enumerate(movies_paths):
        new_name = f"mov{mov_count}_cam{i + 2}_sparse.mat"
